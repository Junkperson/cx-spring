...
public class CXHandlerAdapter {

    public boolean supports(Object handler){ return (handler instanceof CXHandlerMapping);}


    CXModelAndView handle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        CXHandlerMapping handlerMapping = (CXHandlerMapping)handler;
        ...
        boolean isModelAndView = handlerMapping.getMethod().getReturnType() == CXModelAndView.class;
        if(isModelAndView){
            return (CXModelAndView) result;
        }
        return null;
    }
